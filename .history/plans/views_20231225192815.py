...
def settings(request):
    ...
    if request.method == "POST":
        subscription = stripe.Subscription.retrieve(request.user.customers.stripe_subscription_id)
        ...
        try:
            if request.user.customers.membership:
                membership = True
            if request.user.customers.cancel_at_period_end:
                cancel_at_period_end = True
        except Customers.DoesNotExist:
            membership = False
    return render(
